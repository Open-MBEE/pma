...
pmaResponse=$(curl -X POST -H Content-Type:application/json --data "$IP" localhost:8080/projects/$TEAMWORK_PROJECT/refs/master/jobs/$JOB_NAME/instances/$BUILD_NUMBER/jobStatus/$status?mmsServer=${MMS_SERVER})
echo $pmaResponse
